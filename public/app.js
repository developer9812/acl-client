...
var url = 'http://192.168.1.60:81';

_vue2.default.directive('can', {
  bind: function bind(el, binding, vnode) {
    ...
    var permissions = _store2.default.getters.permissions;
    if (permissions == null || permissions.length == 0) {
      var _comment = document.createComment(' ');
      vnode.elm = _comment;
      vnode.isComment = true;
      console.log(vnode);
    ...
    var status = _lodash2.default.some(permissions, ['name', binding.value]);
    console.log("STATUS");
    console.log(status);
    if (!status) {
      var _comment2 = document.createComment(' ');
      vnode.elm = _comment2;
      vnode.isComment = true;
      console.log(vnode);
    } else {}
...
var vm = new _vue2.default({
  ...
  methods: {
    getToken: function getToken() {
      ...
      _axios2.default.post(url + '/oauth/token', {
        'grant_type': 'password',
        'client_id': '1',
    ...
    tryService: function tryService() {
      var _this2 = this;

      this.$store.commit('setLoading', true);
      console.log('TRYING SERVICE');
      _axios2.default.get(url + '/api/users', {
        headers: {
          'Authorization': 'Bearer ' + this.accessToken
        }
      }).then(function (response) {
        console.log(response.data);
        _this2.authError = false;
        _this2.authValid = true;
        _this2.serviceLoaded = true;
        _this2.serviceData = response.data;
        _this2.$store.commit('setLoading', false);
      }).catch(function (error) {
        console.log(error.response.status);
        console.log(error.response.data);
        _this2.authValid = false;
        _this2.accessToken = "TOKEN EXPIRED";
        _this2.serviceLoaded = false;
        _this2.serviceData = [];
        _this2.authError = true;
        _this2.getNewToken();
      });
    },
    getNewToken: function getNewToken() {
      ...
      _axios2.default.post(url + '/oauth/token', {
        ...
        console.log(response.data);
        _this3.authError = false;
        _this3.accessToken = response.data.access_token;
        _this3.refreshToken = response.data.refresh_token;
        _this3.authValid = true;
        _this3.tryService();
      }).catch(function (error) {});
    },
    getRole: function getRole() {
      ...
      _axios2.default.get(url + '/api/user/role', {
        headers: {
          'Authorization': 'Bearer ' + this.accessToken
        }
      }).then(function (response) {
        ...
        console.log(response);
        Role = response.data;
        _this4.$store.commit('setPermissions', response.data[0].permissions);
      }).catch(function (error) {
        console.log("ERROR");
        console.log(error);
      });
